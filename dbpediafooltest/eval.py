...
check_path = "qid_to_ans_backup.json"

#clean_path = "Mixtral_responses_clean.jsonl"
#mixed_path = "Mixtral_responses_mixed.jsonl"
#fab_path = "Mixtral_responses_fab.jsonl"
shuffled_path = "llama2_70b_responses.jsonl"

with open(check_path, "r", encoding="utf-8") as infile:
    qid_to_ans = json.load(infile)


id_to_obj = {}
not_found_count = 0
with open("hottest.jsonl", "r") as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
...
with open(shuffled_path, "r") as infile:
    ...
    for line in infile:
        ...
        if qid in id_to_obj:
            original_answer = id_to_obj[qid]["answers_objects"][0]["spans"][0]
